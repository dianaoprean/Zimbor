    ...
    $.extend($.fn.modal.Constructor.prototype, {
        ...
        initGalleryEvents: function () {
            ...
            modal.find('.modal-image').on('click.modal-gallery', function (e) {
                var modalImage = $(this);
                if ($this.$links.length === 1 && modalImage.children('object').length == 0) {
                    $this.hide();
                } else {
                    // there is an issue on firefox when having a video instead of an image
                    // when click play the video the click event on the modal-image is triggered and the next slide is loaded
                    // that's why we need to check for mozilla and if the displayed item is a video file
                    if (!$.browser.mozilla || modalImage.children('object').length == 0) {
